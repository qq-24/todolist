      ...
      child: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 8),
              child: Text('同步', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: colorScheme.primary)),
            ),
            ListTile(
              ...
              title: FittedBox(
                ...
                child: Text(syncTimeText, style: isError ? TextStyle(color: colorScheme.error, fontSize: 13) : null),
              ),
              trailing: FilledButton.tonal(
                onPressed: todoProvider.syncStatus == SyncStatus.syncing ? null : () => todoProvider.sync(),
            ...
            ListTile(
              ...
              trailing: Text('${todoProvider.completedWishes.length}', style: TextStyle(color: colorScheme.onSurfaceVariant)),
              onTap: () {
                Navigator.of(context).pop();
                Navigator.of(context).push(MaterialPageRoute(builder: (_) => const DoneCollectionScreen()));
              },
            ),
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 8),
              child: Text('排序', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: colorScheme.primary)),
            ),
            RadioListTile<SortMode>(title: const Text('手动排序'), value: SortMode.manual, groupValue: settings.sortMode, onChanged: (v) => settings.setSortMode(v!), dense: true),
            RadioListTile<SortMode>(title: const Text('按截止时间'), value: SortMode.byDeadline, groupValue: settings.sortMode, onChanged: (v) => settings.setSortMode(v!), dense: true),
            RadioListTile<SortMode>(title: const Text('按添加时间'), value: SortMode.byCreatedTime, groupValue: settings.sortMode, onChanged: (v) => settings.setSortMode(v!), dense: true),
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 8),
              child: Text('外观', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: colorScheme.primary)),
            ),
            RadioListTile<ThemeMode>(title: const Text('跟随系统'), value: ThemeMode.system, groupValue: settings.themeMode, onChanged: (v) => settings.setThemeMode(v!), dense: true),
            RadioListTile<ThemeMode>(title: const Text('浅色'), value: ThemeMode.light, groupValue: settings.themeMode, onChanged: (v) => settings.setThemeMode(v!), dense: true),
            RadioListTile<ThemeMode>(title: const Text('深色'), value: ThemeMode.dark, groupValue: settings.themeMode, onChanged: (v) => settings.setThemeMode(v!), dense: true),
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 8),
              child: Text('提醒', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: colorScheme.primary)),
            ),
            SwitchListTile(
              secondary: const Icon(Icons.do_not_disturb),
              title: const Text('勿扰模式时也振动'),
            ...
            if (Platform.isWindows || Platform.isLinux || Platform.isMacOS) ...[
              ...
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 8),
                child: Text('桌面', style: Theme.of(context).textTheme.labelLarge?.copyWith(color: colorScheme.primary)),
              ),
              ...
              if (Platform.isWindows)
                ListTile(
                  ...
                  trailing: Text(HotkeyService.formatForDisplay(settings.hotkey),
